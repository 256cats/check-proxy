...
function testWebsites(proxy, websites) {
  return Promise.map(websites, function(website) {
    var res = testWebsite(website.url, proxy, website.regex, website).reflect();
    return res
  }, {concurrency : 1})
  ...
  return Promise.map(protocolOptions, function(protocolOption) {
    ...
    .spread(function(resHttp, resHttps) {
      if (resHttp.isFulfilled()) {
        ...
        var proxyHttpsData = resHttps.isFulfilled() ? resHttps.value() : null;
        proxyData.supportsHttps = !!(resHttps.isFulfilled() === true && proxyHttpsData);
